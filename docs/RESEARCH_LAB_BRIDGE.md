...
## Producing a bundle (the lab side)

In the Research Lab repository, `hades_research.core_bridge` is the writer.
`CandidateExporter` validates against the same rules Core enforces — fail-closed,
*before* anything is written — and drops the file in the lab's outbox:

```bash
hrl candidate export candidate-spec.json --out ./outbox
hrl candidate verify ./outbox/liquidity-a1b2c3.json   # re-checks the checksum
```

A model that is not a binary logistic fit is **refused** rather than pickled:
`logistic_weights_from_estimator` raises, and no file is written. That model stays
in the lab as a research finding until someone distils it into a weight set.
...
- the lab commits **the same fixture files** and asserts its exporter regenerates
  them byte-for-byte (`tests/test_core_bridge.py` there). Core's test proves the
  bundles *parse*; the lab's proves they are what its writer actually *produces*.
  Neither test alone would catch a writer that quietly stopped emitting them.
